...
$('#container2').highcharts({
    ...
    setupStats: function(viewContext, chart, e){
         setTimeout(function(){
                    ...
                    for (var i = 0; i < self.series.length; i++) {
                        if(self.series[i].name == 'Total patchsets')
                        {
                             ...
                             if(self.series[i].points[h].x>e.min && self.series[i].points[h].x < e.max)
                         {
                                ...
                                sum = self.series[i].points[h].y+sum;
                            }
                        }
                        viewContext.TotalCount['text'] = "Total patchsets " + sum;
                        self.series[i].hide();
                        }
                    ...
                    seriesData = [{name:'success',data:posValue, color:'#8CC610'},{name:'fail',data:negValue, color:'#BD008F' },{name:'missing',data:missValue, color:'#008EFF'}];
                      viewContext.StatsContainer['text'] = 'Stats for ' + Highcharts.dateFormat(null,e.min) + ' - ' + Highcharts.dateFormat(null,e.max);
